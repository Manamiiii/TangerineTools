...
export function ReadingGeoMap({ places, selectedPlaceId, onSelectPlace }) {
  ...
  useEffect(() => {
    ...
    const map = L.map(containerRef.current, {
      ...
      minZoom: READING_MAP_TILE_SOURCE.options.minZoom,
      zoomControl: true,
    }).setView(READING_MAP_DEFAULT_VIEW.center, READING_MAP_DEFAULT_VIEW.zoom)
    const tileLayer = L.tileLayer(
      READING_MAP_TILE_SOURCE.url,
      READING_MAP_TILE_SOURCE.options,
    ).addTo(map)
    const markerLayer = L.layerGroup().addTo(map)

    tileLayer.on('load', () => setTileState('ready'))
    tileLayer.on('tileerror', () => setTileState('error'))
    mapRef.current = map
