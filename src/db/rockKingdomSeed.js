...
// 默认工具仅用于新场景播种；已有场景的工具选择保持原样。
async function seedRockKingdomStructure() {
  const { scene, tables, fields } = ROCK_KINGDOM_PRESET
  await db.transaction('rw', db.scenes, db.catalogTables, db.catalogFields, db.meta, async () => {
    if (!await db.scenes.get(scene.id)) await db.scenes.put(scene)
    for (const table of tables) {
      if (!await db.catalogTables.get(table.id)) await db.catalogTables.put(table)
    }
    for (const field of fields) {
      const existing = await db.catalogFields.where('tableId').equals(field.tableId)
        .filter((item) => item.key === field.key).first()
      if (!existing && !await db.catalogFields.get(field.id)) await db.catalogFields.put(field)
    }
...
async function migrateRockKingdomFieldLayout() {
  ...
  const updates = existingFields.filter(Boolean).flatMap((field) => {
    ...
    if (!preset) return []
    const patch = {}
    if (field.order == null) patch.order = preset.order
    if (field.display == null) patch.display = preset.display || {}
    if (field.tableId === ROCK_KINGDOM_PRESET.tables[0].id && ['shiny', 'shinyImage', 'speciesGroup', 'traitIcon', 'traitDesc'].includes(field.key) && field.hidden == null) patch.hidden = true
...
async function migrateRockKingdomBreedingFieldLabels() {
  ...
  const traitIconField = await db.catalogFields
    ...
    .first()
  if (traitIconField && traitIconField.hidden == null) {
