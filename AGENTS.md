...
TangerineTools is a local-first personal game management web app built with Vite, React 19, and Dexie.js. It is a static frontend app with no backend service. User data lives in browser IndexedDB and is moved through manual JSON export/import.
...
- The default development branch is `main`. Start each development task by inspecting the working tree, switching to `main`, and safely fast-forwarding from `origin/main` when the tree is clean. Develop, verify, commit, and push directly on `main`.
- Do not create a feature branch or pull request unless the user explicitly requests one. If a risky or experimental change would benefit from isolation, explain the reason and obtain the user's approval before creating a branch.
- After completing a development batch and its relevant verification, commit only the scoped changes and push `main` through the configured Git Credential Manager. Report the commit id and push status to the user.
...
- Treat user suggestions as product hypotheses rather than automatic implementation instructions. Evaluate each request against the actual workflow, safety boundaries, existing architecture, and interaction consistency; explain concrete tradeoffs, challenge assumptions when warranted, and propose a better alternative when the requested approach would make the product less coherent. Do not manufacture disagreement for its own sake, and proceed directly when the request remains reasonable after analysis.
