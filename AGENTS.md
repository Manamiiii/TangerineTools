...
- `docs/session-start-prompt.md` stores a lightweight handoff context, code map, and optional short prompt templates for future sessions; do not duplicate stable rules already captured here.
...
- Generated reports should be regenerated by their scripts instead of hand-edited unless the file explicitly says a section is safe for manual notes.
...
- Apply the same final-state standard to generated-report templates and README files. Before finishing a documentation batch, search for stale transition language and either rewrite it as a present-tense constraint or remove it.
...
- For documentation cleanup, prefer clarifying ownership and update triggers over deleting history; keep user-facing README content, agent instructions, generated reports, and handoff prompts distinct.
