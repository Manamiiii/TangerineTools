...
function assignBatch(row, calibrationKeys) {
  ...
  return { batch: '常规批次', focus: '常规样例，按编号核对外部资料' }
}

function batchSortValue(batch) {
  return { '第 1 批': 1, '第 2 批': 2, '第 3 批': 3, '第 4 批': 4, 常规批次: 5 }[batch] || 9
}
...
3. **逐只推进**：按捕捉进度一次核对一只精灵；批次字段只用于筛选和专题回归。
...
|---|---|---|---|---|
| 0. 指定目标精灵 | 用户 | 编号/名称/形态、捕捉目标、是否已有候选性格 | 单只核对范围 | 否。 |
...
| 3. 捕捉取舍建议 | Codex Cloud | 本地 + 外部定位 | 推荐性格、可保留性格、明确不保留性格、原因和风险 | 否。 |
...
| 已核对-需调整 | 规则、标签或解释存在已确认问题。 |

## 批次定义（专题回归索引，不是日常处理顺序）

批次字段用于发现多只精灵共有的规则问题和组织专题回归。日常核对按用户给出的编号、名称和形态推进。
...
| 常规批次 | ${batchCounts['常规批次'] || 0} | 常规样例，按编号核对外部资料。 |

## 每只精灵核对字段

| 字段 | 说明 |
|---|---|
| 批次 | 按校准样例、低生命高单防、综合肉度、速度线和常规样例分类，用于筛选专题问题。 |
...
定位核对只接受洛克王国世界资料，不照抄外部推荐性格。待人工确认项集中在迅捷启动与站场机制、资料不足和外部口径分歧。
