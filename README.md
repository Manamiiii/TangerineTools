...
- **阅读伴侣**：内置「经典文学阅读」场景，通过书架选择指定译本并保存阅读章节；粘贴段落可在本机扫描其中实际出现的已审计名称，已解锁真实地点显示在可缩放互动地图中；底图可在免 Key 的国际地图与需要浏览器端 Key 的国内地图之间切换，资料包外地点可经公网搜索并由读者确认个人位置，同时以确定性规则约束资料的可揭示进度和剧透确认级别。
...
阅读伴侣按当前章节过滤正式资料包中的实体和事实。已审计真实地点可显示在 Leaflet 互动地图中，OpenStreetMap 在线瓦片提供道路和区域背景；底图不可用时仍保留地点坐标与详情。地图支持列表联动、自动适配视野，并计算已读地点之间的方向和直线距离。安全事实直接显示，潜在剧透需要确认，高风险内容需要二次确认；授权不会持久化。正式地点和事实必须引用已批准来源与指定版本的章节边界；开发时研究候选与测试夹具不属于运行时正式资料。
