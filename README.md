...
- **阅读伴侣**：Windows 首版可安装为 PWA，从书架选择不可删除的内置资料包，或凭版本信息、目录与自选封面建立可删除的个人书籍；书内固定提供阅读输入、分类分页的已遇到记录、地图和设置。章节在书籍页右上角选择；粘贴或 OCR 后自动清理排版并执行本机已知名称扫描与轻量人名/地名候选识别，也可选中文字快速记录。智谱、DeepSeek、MiniMax、OpenAI 与自定义兼容接口提供可选模型候选识别和国际地图英文搜索词转换；真实地点可经国内/国际公网地图搜索，虚构或模糊地点可由读者设置宽泛参考区域，地图支持点、路径、范围与全屏。设置页集中管理模型和地图 Key、数据边界与接入教程，同时以确定性规则约束资料的可揭示进度和剧透确认级别。
