...
| `scripts/data/bwiki/creature-details.staging.json` | 精灵详情页完整 staging | 特性描述、技能卡关系、血脉 / 技能石标签、进化链验证；旧模板使用官方 API 源码回退 |
...
| `form` | 形态 | `formCategoryLabel`、`isMainForm`、`stageLabel`、名称差异 | BWiki 形态分类只作辅助；已匹配行保留本地精确 form，新增行再按名称括号、阶段、分类依次推导 | 写入候选并报告来源策略，不静默泛化覆盖 |
...
当前 preview 已完成 592 条精灵详情审阅，并已将 `id` / `values` 显式发布到 `public/presets/*`，同时发布旧官方值 SHA-256 迁移清单。下一步先做已有浏览器升级走查；后续数据更新仍必须先生成 staging / preview 报告并通过显式发布命令落盘。
