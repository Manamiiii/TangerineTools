...
- 预置资料仍以官方 `d.json` / 本地可信 `scripts/data/rockKingdom.d.json` 为准。孵蛋推荐默认只分析异色拥有个体；蛋组/同种精灵补充数据目前只写入已核对的 B 站蛋组计算器可见条目（当前覆盖「无法孵蛋」和「动物组」可见条目），后续扩大覆盖面时仍需标注外部来源且不得覆盖用户自定义非空字段。
