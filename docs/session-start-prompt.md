...
- BWiki P4 已正式覆盖 `public/presets`，发布 592 条精灵、553 条技能及三方迁移清单。迁移只更新空值、无效值或仍匹配旧官方指纹的字段，保留用户非空自定义值、旧 id、用户新增行及 owned / stock 数据；运行时不在线访问 BWiki。下一步先做已有浏览器升级走查，再进入 P5 洛克王国精灵详情分块 UI。
