...
- 预置资料仍以官方 `d.json` / 本地可信 `scripts/data/rockKingdom.d.json` 为准。孵蛋推荐默认只分析异色拥有个体；蛋组/同种精灵补充数据优先读取 `public/presets/rockKingdomBreedingRows.json` 的 BWiki 快照，快照可由 `npm run sync:breeding` 从 B 站精灵图鉴详情页生成；迁移只补空值，不得覆盖用户自定义非空字段。
