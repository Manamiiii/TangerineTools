...
退役的 gamecenter `d.json`、旧同步器和阶段性发布报告已删除。需要版本审阅的性格 / 定位报告统一进入 `docs/generated/`；BWiki 临时检查报告进入被 Git 忽略的 `artifacts/`。
...
scripts/bwiki/                # BWiki 脚本、共享转发层、staging 与 preview
scripts/tests/                # 纯逻辑测试和 fake-indexeddb 集成测试
docs/nature/                  # 性格规则、单只模板和人工确认台账
docs/generated/               # 由脚本重建并随版本审阅的报告
...
- BWiki 数据生产线已集中到 `scripts/bwiki/`，来源与字段血缘文档集中到 `docs/data-sources/`；npm 命令名保持不变。
- 性格文档按“人工规则 / 人工台账 / 版本化生成报告”分层；RoCom 人工核对结论改由 `scripts/data/rocomAuditFindings.json` 维护。
