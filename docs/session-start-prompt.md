...
- BWiki P4 已正式覆盖 `public/presets`，发布 592 条精灵、553 条技能及三方迁移清单；后续优化已补齐新增精灵标签和繁育字段、切换 BWiki 无文字系别图标，并以只读归并视图隐藏旧概括行/随机 id 精确重复行。迁移仍只更新空值、无效值或匹配旧预置指纹的字段，保留用户非空自定义值、旧 id、用户新增行及 owned / stock 数据；运行时不在线访问 BWiki。下一步先在另一台电脑走查优化分支的重复行、图标和表格布局，再进入 P5 洛克王国精灵详情分块 UI。
