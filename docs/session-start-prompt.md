...
- 预置资料仍以官方 `d.json` / 本地可信 `scripts/data/rockKingdom.d.json` 为准。孵蛋推荐默认只分析异色拥有个体；B 站等外部资料只作为官方资料缺失字段（如蛋组、繁育谱系）的校正和补充来源。`public/presets/rockKingdomBreedingRows.json` 是 BWiki 补充快照，可由 `npm run sync:breeding` 重新生成；迁移只补空值，不得覆盖官方/用户已有非空字段。
