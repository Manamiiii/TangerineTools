...
详情链接点击只等待实际点击完成；目标 URL、正文身份、文档就绪和稳定性统一由本地读取检查负责，不额外等待可能延迟的导航事件。正文等待预算内不会因独立的三十秒 `DOMContentLoaded` 等待提前终止。

打开或返回图鉴时只等待导航提交，随后由本地短次读取验证精确地址、唯一图鉴容器及全部解析条目。解析结果连续两秒稳定且与同批次缓存一致后才允许下一次点击。导航事件等待超时只触发对当前页面的有界检查，不再次导航；错误地址、内容变化、HTTP 错误或访问验证不能通过该检查。`returning` 状态记录返回图鉴阶段及其地址，避免把返回错误归到已成功保存的详情。

图鉴就绪检查允许卡片在六十秒内从半成品补齐；暂时缺名称、解析失败或条目不全会重置稳定计时，只重新读取当前 DOM。到期仍不完整或与缓存不同则保留最后的校验原因并停止，不能使用部分条目继续采集。HTTP 错误和访问验证不进入该等待流程。

对于只渲染屏幕附近卡片的图鉴，命令读取浏览器本次导航已经收到的原始 HTML 响应，不额外请求页面。仅在原始响应全部字段与同批次缓存完全一致时，允许当前 DOM 中的 `stageLabel`、`image`、`shinyImage` 为空；名称、编号、sourceId、链接及其他字段仍须逐项一致，非空阶段或图片差异也不得忽略。响应未完整取得或原始资料变化时不能通过该检查，不从缓存拼补采集产物；详情仍通过实际卡片点击及 DOM 双遍校验保存。

详情读取前由本地进程每半秒短次读取页面，确认 URL、唯一正文和目标 `data-pet-id`，并等待正文完整 HTML 连续十秒不变。等待不依赖网页内部的长时间 Promise，页面自身导航时清空稳定性计时。就绪后执行独立的身份检查、两秒稳定性检查和双遍逐字比对，读取前后检查文档导航序号，禁止混合不同文档的片段。导航中断或正文变化时丢弃整次读取，在同一页面重新等待，最多三次读取、总体等待预算两分钟；不点击、刷新或重新请求网址，不对采集内容删除动画属性或改写数值。等待期间发现访问验证或 HTTP 错误立即停止，不纳入本地重读。

长时间采集使用独立进程运行，并将标准输出和错误输出重定向至该批次的 `collector-时间.output.log` / `.error.log`。PowerShell 可使用 `Start-Process -WindowStyle Hidden -PassThru` 启动本机 Node 和上述脚本参数；核对实际命令行及新日志，不把旧状态文件或单页成功视为持续运行的证据。
...
`node --test scripts/tests/nrc-shiny.test.mjs` 检查火红尾、雅丹鬃确认的批次与身份范围、旧来源保留、原始记录不变，以及无异色时拒绝回填旧图。
