...
当前正式数据采用 NRC；采集快照、确认范围和来源差异见 [`nrc-source.md`](nrc-source.md)。以下通用字段映射适用于两个来源，NRC 的来源专属口径以该文档为准。技能“防御”在当前转换器中归入 `status`；这类表示差异不属于版本平衡调整。

## 输入与输出范围

| 输入 | 产物 | 当前用途 |
|---|---|---|
| `scripts/bwiki/data/nrc/staging/*.json` | NRC 正式来源 staging | 同批次图鉴、技能、详情与蛋组；对应 preview 位于 `nrc/preview/` |
| `scripts/bwiki/data/staging/creatures.json` | rocom 精灵筛选页 staging | 精灵基础信息、六维、形态、图片入口、蛋 / 果实图片、详情页链接 |
...
| `public/presets/rockKingdomRows.json` | 当前精灵预置 | 正式基线 621 行 |
| `public/presets/rockKingdomSkillRows.json` | 当前技能预置 | 正式基线 579 行 |
...
| `bst` / `hp` / `patk` / `matk` / `pdef` / `mdef` / `spd` | 种族值 / 六维 | rocom 的 `creatures.rows[]` / NRC 的详情 `stats` | NRC 必须读取已校验详情六维及总和；直接数值映射；空值或非数字进入报告 | 写入 preview |
...
| `skillRefs` | 可用技能 | 详情 `skills[].name` 与 `skills[].sourceType` | 按技能名称匹配 preview 技能行 id；NRC 合并 level / machine / blood / legendary 并去重；保留来源标签用于审计，现有字段只存 row id 数组 | 写入 preview；输出未匹配技能名 |
| `eggGroups` | 蛋组 | `sync:breeding` 版本化快照 | 精灵筛选页 `data-param8` 是归属赛季而不是蛋组；稳定匹配行保留非空值，空值或新行按安全名称匹配从孵蛋快照补齐 | 写入 preview 候选并报告来源 |
| `speciesGroup` | 繁育谱系 | 当前孵蛋快照 / 名称归并 | 保持文本字段；保留已有家族，其次采用蛋组快照；NRC 缺失时以身份匹配的进化分支初始形态补齐，冲突阻止发布 | 写入 preview 候选 |
...
- 精灵 preview 应优先复用现有 `rock-creature-src-*` 稳定 id，但只在“编号 + 名称”或唯一名称可确认匹配时复用；同编号但名称不同的 BWiki 形态 / 新增精灵必须分配新稳定 id，避免覆盖普通形态。明确的用户改名确认可按指定批次、sourceId、编号和旧 ID 精确复用，范围见 NRC 文档。
