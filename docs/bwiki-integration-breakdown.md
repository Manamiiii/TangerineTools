...
P4 已正式发布 592 条精灵、553 条技能和版本化迁移清单；本轮又补齐全部新增精灵的特性标签、技能标签、蛋组和繁育谱系，并改用 BWiki 无文字系别图标。迁移清单目前涉及 592 条精灵的 3015 个字段和 487 条技能的 1615 个字段，29042 条技能关系双向一致且无悬空引用。历史 29 个旧 id 与旧随机 id 不删除，在对应新版行存在时从资料库、引用选择、性格推荐和统计视图隐藏。下一步由用户在另一台电脑拉取优化分支，重点走查重复行、系别图标、统一行高和特性组合展示；确认后再进入 P5 洛克王国精灵详情分块 UI。
