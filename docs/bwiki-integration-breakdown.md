...
P4 已正式发布 592 条精灵、553 条技能和版本化迁移清单；后续优化已补齐标签与繁育字段、改用 BWiki 无文字系别图标，并隐藏有对应新版行的历史 id / 随机 id 重复行。本轮使用 BWiki `formCategoryLabel` 校正 61 条首领形态（包括烈火战神），迁移清单现涉及 592 条精灵的 3065 个字段和 487 条技能的 1615 个字段，29042 条技能关系双向一致且无悬空引用。下一步由用户在另一台电脑拉取优化分支，重点走查资料库/性格选择器的形态顺序、选择器过滤和最终形态 + 首领形态联合分析；确认后再进入 P5 洛克王国精灵详情分块 UI。
