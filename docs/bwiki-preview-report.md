# BWiki preset preview report

Generated at: 2026-07-17T07:11:18.816Z
...
| Kind | Count / path |
|---|---:|
| BWiki creature staging rows | 592 |
| BWiki skill staging rows | 553 |
| BWiki detail staging rows | 4 |
| Current public creature preset rows read | 496 |
| Current public skill preset rows read | 487 |
| Creature preview output | `scripts/data/bwiki/rockKingdomRows.preview.json` (592 rows) |
| Skill preview output | `scripts/data/bwiki/rockKingdomSkillRows.preview.json` (553 rows) |
...
- NO.120 暮风隐者（枯叶的样子） → rock-creature-bwiki-8af36cfaf632
- NO.189 棋契陛下（白棋棋骑士分支） → rock-creature-bwiki-1222f8112a99
- NO.189 棋契陛下（黑棋棋骑士分支） → rock-creature-bwiki-fae4f0b735b7
- NO.190 棋契陛下（白棋棋绮后分支） → rock-creature-bwiki-95a0fdc4aff2
- NO.190 棋契陛下（黑棋棋绮后分支） → rock-creature-bwiki-99ed8ddb7fc5
- NO.191 棋契陛下（白棋棋齐垒分支） → rock-creature-bwiki-edf7620c19d6
- ……另有 73 项
...
- NO.001 圣光迪莫：bst 582 → 596
- NO.001 圣光迪莫：hp 120 → 122
- NO.001 圣光迪莫：patk 80 → 84
- NO.001 圣光迪莫：matk 80 → 84
- NO.001 圣光迪莫：pdef 105 → 107
- NO.001 圣光迪莫：mdef 105 → 107
- NO.001 圣草迪莫：bst 582 → 594
- NO.001 圣草迪莫：hp 120 → 132
- NO.001 圣火迪莫：bst 582 → 598
- NO.001 圣火迪莫：patk 80 → 88
- NO.001 圣火迪莫：matk 80 → 88
- NO.001 圣水迪莫：bst 582 → 598
- NO.001 圣水迪莫：pdef 105 → 113
- NO.001 圣水迪莫：mdef 105 → 113
...
- NO.001 圣草迪莫：light → grass、light
- NO.001 圣火迪莫：light → fire、light
- NO.001 圣水迪莫：light → water、light
...
- NO.031 恶魔男爵：dark、flying → flying、dark
