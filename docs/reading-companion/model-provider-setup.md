...
阅读伴侣通过 OpenAI Chat Completions 兼容协议调用外部模型。模型可以从读者主动放入的当前小段中发现人物、地点、概念和事件名称，为个人书籍自动准备隐藏名称词典，解释当前概念或段落，整理书目信息并提供地图检索词。只有把名称写入“已遇到”需要读者确认当前原文确实出现该名称；个人书籍词典不要求未读用户逐条审核，答疑结果只临时显示。模型不能直接生成正式事实、关系或坐标。
