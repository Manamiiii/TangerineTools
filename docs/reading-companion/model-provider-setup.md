...
硅基流动是国内模型聚合平台。免费模型名单和限流可能调整，应在模型广场确认当前的“免费”标记。
...
- 实名认证后可以调用平台当前提供的免费模型；免费模型的账单调用费用为 0，但有固定频率限制。
- 如果建议模型已经下线或不再免费，在工具中直接填写模型广场展示的新 ID。
- 在[硅基流动 API Key 管理](https://cloud.siliconflow.cn/account/ak)创建 Key。
- 免费规则见[硅基流动 Rate Limits](https://docs.siliconflow.cn/cn/userguide/rate-limits/rate-limit-and-upgradation)。

## 阿里云百炼

适合测试通义千问。新人免费额度不是长期免费额度。
...
- 中国内地部署的部分模型在首次开通后提供新人额度，通常自开通起 90 天有效；不同模型额度独立。
- 建议在百炼控制台开启“免费额度用完即停”，避免额度用尽后自动转为按量计费。
- 百炼推荐新应用使用业务空间专属域名。拿到专属地址后，可以直接覆盖工具中的兼容地址：
...
- 额度规则见[百炼新人免费额度](https://help.aliyun.com/zh/model-studio/new-free-quota/)，接口格式见[OpenAI 兼容 Chat](https://help.aliyun.com/zh/model-studio/qwen-api-via-openai-chat-completions)。
