...
- 对照用户确认结果和版本化截图，53 张图鉴名与字段通过；原第 46、50 张仍受正式资料库缺项阻塞，见下节。
...
- 原第 46、50 张的用户确认名称分别为“漏斗鸭”“领结鸭”，但当前正式资料库和 BWiki 预设均没有对应行。扫描器只能关联到现有鸭吉吉候选，不能生成一个不存在的引用。补齐正式资料行后，这两张才能成为可保存且可回归的正确结果；
...
| 45 | 血脉：污染 | 血脉误识别 |
| 46 | 名称：漏斗鸭 | 名称或形态误识别；图鉴名与精灵名口径待后续确认 |
| 48 | 图鉴名：鸭吉吉（紧实的样子） | 形态未识别 |
| 50 | 名称：领结鸭 | 名称或形态未识别；图鉴名与精灵名口径待后续确认 |
