...
`scripts/tests/rock-kingdom-scanner.test.mjs` 使用本表的版本化截图验证字段级字形模板、固定设备裁切和通用匹配门槛。`scripts/data/rockKingdomScannerTextTemplates.json` 只登记字段、合法选项和来源截图，不登记“某只精灵应当是什么血脉/性格”的精灵特例。当前自动回归覆盖 18 种性格、15 种血脉和特长「奇袭」，并验证：
