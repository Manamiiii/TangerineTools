...
本文件用于核对固定手机视频的智能抽帧、字段识别和性能变化。表内“当前识别”是扫描器的初次实测输出，不等同于正确答案；后续回归以“用户确认基线”为期望值，并保留初次输出用于定位差异。
...
- 智能抽帧：检查 318 个时间点，步长约 0.30 秒，保留 55 张，耗时 163.7 秒；
- 第 25、26 张的同一画面在提取阶段合并，后续序号从原第 27 张起前移一位；
- 批量识别：55 张，总耗时 22.4 秒；
- 所有 55 张均完成自动关联，没有“完全未匹配”或“同名形态待选择”状态；
- 对照用户确认结果和版本化截图，55 张图鉴名与字段全部通过。

性能比较必须区分首次加载 OCR 模型的冷启动与模型已经加载的热运行。本表记录的是热运行；单帧状态中的时间只计算该帧识别，不包含智能抽帧、人工复核和模型纠错。

## 当前端到端结论

固定视频已经完成一次从原始 HEVC 文件重新提取、批量识别到逐字段比对的浏览器回归：

- 自定义名“梦梦”精确关联到“梦悠悠（穿旧睡衣的样子）”，同时保留昵称；
- 板板壳、地鼠、罗隐、燃薪虫、喵喵以及已有资料库记录的鸭吉吉形态均能自动关联；
- 血脉、性格、特长、伙伴标记、外观和性别与用户确认结果及截图一致；
- 左侧选中白圈按固定设备网格识别，稀有外观的彩色描边不再充当选中证据；
- 原第 46、50 张的“沙漏”“领结”是鸭吉吉的外观描述，不是独立图鉴名。结合等级、星级、性格和六维公式，它们分别唯一对应“鸭吉吉（等一等鸭）”和“鸭吉吉（急急急鸭）”；
- 用户确认表把原第 19 张血脉记为“恶”，但版本化截图显示该帧为“阿米亚特 / 地”。当前回归以截图中的可见终态为准，保留此差异供用户最终确认。

## 用户确认基线

用户已按顺序核对全部 56 个结果。下表只记录用户报告的差异和抽帧异常；没有列出的序号表示初次识别数据确认无误。字段未在“确认结果”中提及，表示沿用逐帧结果里的初次识别值。

| # | 确认结果 | 问题分类 |
|---:|---|---|
| 3 | 血脉：水 | 血脉误识别 |
| 4 | 血脉：地 | 血脉误识别 |
| 5 | 精灵名：梦梦；图鉴名：梦悠悠；血脉：幽 | 自定义名称、图鉴名和血脉未识别 |
| 6 | 识别数据无误 | 截图为空白，截图资产不可用于视觉复核 |
| 7 | 血脉：武 | 血脉误识别 |
| 9 | 血脉：毒 | 血脉误识别 |
| 10 | 血脉：光 | 血脉误识别 |
| 11 | 血脉：光 | 血脉误识别 |
| 15 | 血脉：光 | 血脉误识别 |
| 17 | 血脉：翼 | 血脉误识别 |
| 18 | 精灵名、图鉴名：罗隐；血脉：恶 | 名称和血脉误识别 |
| 19 | 血脉：恶 | 血脉误识别 |
| 20 | 识别数据无误 | 截图异常，截图资产不可用于视觉复核 |
| 22 | 血脉：武 | 血脉误识别 |
| 23 | 图鉴名：燃薪虫；血脉：草 | 图鉴名和血脉未识别 |
| 25–26 | 两张为同一画面；26 不应作为新的精灵画面 | 重复抽帧 |
| 27 | 血脉：奇异 | 血脉误识别 |
| 28 | 血脉：污染 | 血脉误识别 |
| 29 | 血脉：奇异 | 血脉误识别 |
| 30 | 图鉴名：喵喵；血脉：草 | 图鉴名和血脉未识别 |
| 31–33 | 血脉：草 | 血脉误识别 |
| 34–37 | 血脉：火 | 血脉误识别 |
| 39 | 血脉：奇异 | 血脉误识别 |
| 44 | 图鉴名：鸭吉吉（蓬松的样子） | 形态未识别 |
| 45 | 血脉：污染 | 血脉误识别 |
| 46 | 图鉴名：鸭吉吉（等一等鸭）；外观像沙漏 | 形态误识别 |
| 48 | 图鉴名：鸭吉吉（紧实的样子） | 形态未识别 |
| 50 | 图鉴名：鸭吉吉（急急急鸭）；外观戴领结 | 形态未识别 |
| 53–54 | 图鉴名：鸭吉吉（紧实的样子） | 形态未识别 |
| 55–56 | 图鉴名：鸭吉吉（蓬松的样子） | 形态未识别 |

本样本的抽帧期望不能只检查“56 张”：第 6、20 张的复核截图异常，第 25、26 张重复。后续优化应保留对应精灵的稳定终态画面，并消除重复画面；在此之前，性能对比仍以本次 56 张初始输出为参照。

## 自动回归覆盖

`scripts/tests/rock-kingdom-scanner.test.mjs` 使用本表的版本化截图验证字段级字形模板、固定设备裁切和通用匹配门槛。`scripts/data/rockKingdomScannerTextTemplates.json` 只登记字段、合法选项和来源截图，不登记“某只精灵应当是什么血脉/性格”的精灵特例。当前自动回归覆盖 18 种性格、15 种血脉、特长「奇袭」和名称「燃薪虫」；47 张具有明确选中白圈的截图提供头像证据，其余 5 张不使用弱选中证据。回归同时验证：

- seek 后必须等待目标解码帧实际呈现；
- 左侧选中格变化但右侧细节未变化的失败切换只保留一张；
- 连续网格选中位置按白圈依次移动，不受异色、炫彩描边干扰；
- 右侧细节不同的连续同种精灵分别保留；
- 短文字 OCR 不能凭单个共同字符强配到较长选项；
- 首轮名称 OCR 很弱时，有限词表重试可以恢复明确领先的图鉴名；
- 鸭吉吉和板板壳等同名形态继续通过等级、星级、性格与六维的通用公式约束区分。

`npm run check:scanner:templates` 校验所有生成模板与截图来源一致，`npm test` 执行上述行为和图像回归。视频端到端的保留帧数量、实际浏览器 OCR 结果和耗时仍以本表的人工复核流程为准。

## 逐帧结果

“未选择”表示扫描器没有得到可采用的结果。“无”可能是明确识别为无，也可能是字段默认值。表末“用户确认或纠正”列是初次测试时的占位状态，权威确认结果以“用户确认基线”为准。
...
| 46 | 1:19.5 | [打开](assets/rock-kingdom-scanner/baseline-3200x1440/frame-46-079500ms.webp) | 鸭吉吉（等一等鸭） | 调皮 | 普通系 | 无 | 无 | 房屋 | 母 | 已确认 / 1.9 秒 | 沙漏外观；公式误差 0 |
...
| 50 | 1:25.8 | [打开](assets/rock-kingdom-scanner/baseline-3200x1440/frame-50-085800ms.webp) | 鸭吉吉（急急急鸭） | 天真 | 普通系 | 无 | 无 | 房屋 | 母 | 已确认 / 1.3 秒 | 领结外观；公式误差 0 |
...
1. 是否保留去重后的 55 张，以及精确时间点是否发生非预期漂移；
