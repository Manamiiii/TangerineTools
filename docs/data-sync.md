...
`src/presets/rockKingdom.js` 的 `ROCK_KINGDOM_ROWS_VERSION` 必须与正式迁移清单的 `version` 一致；每次发布同时更新，已有浏览器才会触发对应迁移。迁移清单必须可用且版本一致，目标数组长度必须与清单一致；失败时不写入完成标记，后续启动重试。快速启动同时要求运行时版本和清单验证版本匹配，缺少验证标记时执行安全三方合并。数据库集成测试检查两者一致及同版本不重复加载。
...
收集记录的单条、批量和扫描写入共用事务校验。单条模式下新增同一引用更新原记录，编辑冲突和扫描追加冲突则拒绝；批量失败整体回滚。洛克王国异色约束适用于所有写入入口，导入恢复仍保持按 id 合并语义。
...
- 资料表需要 `sceneId`，字段需要 `tableId`、`key`、`type`，行需要 `tableId` 和对象形式的 `values`。字段选项兼容字符串形式，导入和启动时将字符串规范化为同值同标签的选项对象；未知扩展属性和行值保持原样。
