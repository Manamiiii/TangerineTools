...
- 精灵行数据通过 `fetch(`${BASE_URL}presets/rockKingdomRows.json`)` 拉取，技能行数据通过 `fetch(`${BASE_URL}presets/rockKingdomSkillRows.json`)` 拉取，孵蛋辅助资料通过 `fetch(`${BASE_URL}presets/rockKingdomBreedingRows.json`)` 拉取，文件放在 `public/` 下，不参与 JS 打包。拉取失败（如离线）时仅打印警告，不阻塞场景/表/字段骨架。
...
- B 站精灵图鉴是 WIKI 页面数据快照（页面自身显示更新日期），不是本应用运行时实时查询接口。维护者可运行 `npm run sync:breeding` 从 B 站精灵图鉴详情页生成 `public/presets/rockKingdomBreedingRows.json`。应用启动迁移只用该快照补齐空值，不在用户浏览器里实时抓取 B 站。
- 预置资料迁移策略：
