...
- B 站精灵图鉴是 WIKI 页面数据快照（页面自身显示更新日期），不是本应用运行时实时查询接口；它不替代官方 `d.json`。维护者可运行 `npm run sync:breeding` 从 B 站精灵图鉴详情页生成 `public/presets/rockKingdomBreedingRows.json`，只作为蛋组/繁育谱系补充快照。应用启动迁移只用该快照补齐空值，不在用户浏览器里实时抓取 B 站。
- 预置资料迁移策略：
